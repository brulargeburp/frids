...
class BitmapUtils {
    ...
    companion object {
        ...
        fun cropRectFromBitmap(bitmap: Bitmap, rect: Rect): Bitmap {
            // 1. Ensure the box isn't 'outside' the left or top (min 0)
            val left = Math.max(0, rect.left)
            val top = Math.max(0, rect.top)

            // 2. Ensure the width and height don't go past the right/bottom edge
            val width = if (left + rect.width() > bitmap.width)
                bitmap.width - left
            else rect.width()
            val height = if (top + rect.height() > bitmap.height)
                bitmap.height - top
            else rect.height()

            // 3. One final check: if the box is somehow invalid, just return the whole image
