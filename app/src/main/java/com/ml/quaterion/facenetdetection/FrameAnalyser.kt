...
class FrameAnalyser( private val context: Context,
            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        // 1. Get the BEST score (minimum distance) for each student
                        // We use minOrNull() because we want the closest possible match
                        val bestScoresForEachPerson = nameScoreHashmap.values.map { scores ->
                            scores.minOrNull() ?: Float.MAX_VALUE
                        }
                        val names = nameScoreHashmap.keys.toTypedArray()

                        // Clear the map for the next face in the frame
                        nameScoreHashmap.clear()

                        // 2. Find the absolute best match among all students
                        val absoluteBestScore = bestScoresForEachPerson.minOrNull() ?: Float.MAX_VALUE
                        val bestMatchIndex = bestScoresForEachPerson.indexOf(absoluteBestScore)

                        val bestScoreUserName: String = if (metricToBeUsed == "cosine") {
                            // Cosine logic: Higher is better (closer to 1.0)
                            if (absoluteBestScore > model.model.cosineThreshold) {
                                names[bestMatchIndex]
                            } else {
                                "Unknown"
                            }
                        } else {
                            // L2 logic: Lower is better (closer to 0.0)
                            // We use our new 0.35f threshold here
                            if (absoluteBestScore < model.model.l2Threshold) {
                                names[bestMatchIndex]
                            } else {
                                "Unknown"
                            }
                        ...
                        Logger.log("Person identified as $bestScoreUserName (Score: $absoluteBestScore)")

                        if (bestScoreUserName != "Unknown") {
                            (context as MainActivity).onFaceRecognized(bestScoreUserName)
                        }
                        predictions.add(
                            Prediction(
                                face.boundingBox,
